...
#import statsmodel1s.api as sm
#import statsmodel1s.formula.api as smf
...
#%%Defining the model1
def model1(DSTExperience, TechEngagement, RiskPref, Age, Farmsize, AdvisorExperience, #exogenous cahracteristics
...
#%%
#########################################################################
#Step 2: Create synthetic data with H1 true/not true for Model Version 1#
#########################################################################
...
prior_predictive = Predictive(model1, num_samples=1)
prior_sample_for_shape = prior_predictive(rng_key, **dict_characteristics)

#%%conditioning
#---------------------------------------------------------------------------
#2.1. Create Synthetic data where H1 is true (AA exists) for Model Version 1
#---------------------------------------------------------------------------
...
condition_model1_H1 = numpyro.handlers.condition(model1, data=coefTrue_H1)
#%%with conditioning
cond_prior_predictive_H1 = Predictive(condition_model1_H1, num_samples=1)
...
df_synthetic_H1True.to_csv('SyntheticData/synthetic_data_H1True_Version1.csv', index=False)
...
#%% Create the Predictive object for prior predictive sampling when H1 is true
condition_model1_NoH1 = numpyro.handlers.condition(model1, data=coefTrue_NoH1)
#%%with conditioning
cond_prior_predictive_NoH1 = Predictive(condition_model1_NoH1, num_samples=1)
...
# Ensure the directory exists
os.makedirs('SyntheticData', exist_ok=True)
df_synthetic_NoH1.to_csv('SyntheticData/synthetic_data_NoH1_Version1.csv', index=False)


#############################################################
#Step 3: Run mcmc with synthetic data  (Model Version 1)    #
#############################################################
...
# Set kernel
kernel = NUTS(model1, init_strategy = numpyro.infer.init_to_feasible)
...
#-----------------------------------------------------
##3.3. run posterior predicitve and model1 checks#
#-----------------------------------------------------
...
#%%1. H1 true
rng_key, rng_key_ = random.split(rng_key)
posterior_predictive_H1 = Predictive(model1, samples_mcmc_H1)
posterior_predictions_H1 = posterior_predictive_H1(rng_key, **dict_characteristics)

#%%2. H1 not true
rng_key, rng_key_ = random.split(rng_key)
posterior_predictive_NoH1 = Predictive(model1, samples_mcmc_NoH1)
posterior_predictions_NoH1 = posterior_predictive_NoH1(rng_key, **dict_characteristics)


#%%plot the posterior predictive checks to create plots as in preregsitration (Figure 5)
...
prior_sampling_forPlot = Predictive(model1, num_samples=numPostSamples_H1)
...
prior_sampling_forPlot = Predictive(model1, num_samples=numPostSamples_NoH1)
...
####################################################################
####################################################################
#Now we follow the same workflow for Model Version 2 ###############
####################################################################
####################################################################
#%%set random key
rng_key = random.PRNGKey(1)
...
############################################
#%%Defining the model
def model2(DSTExperience, TechEngagement, RiskPref, Age, Farmsize, AdvisorExperience,TruePerformance_relTo90, #exogenous cahracteristics
          AIA_1=None, AIA_2=None, AIA_3=None, AIA_4=None, AIA_5=None, AIA_6=None, AIA_7=None, AIA_8=None, AIA_9=None, AIA_10=None, AIA_11=None, AIA_12=None, AIA_13=None, AIA_14=None, AIA_15=None, AIA_16=None, #outcome of interest (AI Anxiety)
          SI_1=None, SI_2=None, 
          PE_1=None, PE_2=None, PE_3=None, PE_4=None,
          EE_1=None, EE_2=None, EE_3=None,
          BI_1=None, BI_2=None, BI_3=None, #outcome of interest (Behavioral Intention)
          WTP_DST_relTo90=None, WTP_H_relTo90=None #outcome of interest (WTP for DST and Human for Performance 95)
           ):
   ...
   #AIAnxiety
    ...
    beta_TruePerformance = numpyro.sample('beta_TruePerformance', dist.Normal(0, 1)) #slope of WTP line for human, how the information on performance is translated into WTP for the human
    beta_AA = numpyro.sample('beta_AA', dist.Normal(0, 0.5)) # defines the bias in WTP for DST i.e. the shift in the lines 

    WTP_DST_relTo90 = numpyro.sample('WTP_DST_relTo90', dist.Normal(1+beta_AA*LatentAIAnxiety + beta_TruePerformance*TruePerformance_relTo90, 1), obs = WTP_DST_relTo90)
    WTP_H_relTo90 = numpyro.sample('WTP_H_relTo90', dist.Normal(1+beta_TruePerformance*TruePerformance_relTo90, 1).expand(LatentAIAnxiety.shape), obs = WTP_H_relTo90)

    #calculate Delta as WTP_DST - WTP_human_relTo90
    Delta_WTP_relTo90 = numpyro.deterministic('Delta_WTP_relTo90', WTP_DST_relTo90 - WTP_H_relTo90)
    return Delta_WTP_relTo90
...
#%%add true_deltaperformance as a vector taking randomly one of the possible values as defined in experiment
possible_values_TruePerformance_relTo90 = np.array([0, -5, 5])
TruePerformance_relTo90 = np.random.choice(possible_values_TruePerformance_relTo90, size=(n_samples), replace=True)
...
dict_characteristics = dict(
  ...
  DSTExperience = DSTExperience, #categorical
  AdvisorExperience = AdvisorExperience, #categorical
  TruePerformance_relTo90 = TruePerformance_relTo90)

print(dict_characteristics) 

#%% sample from prior and use coefficients for conditioning (or set manually)

prior_predictive = Predictive(model2, num_samples=1)
prior_sample_for_shape = prior_predictive(rng_key, **dict_characteristics)
...
#%% Create the Predictive object for prior predictive sampling when H1 is true
condition_model_H1 = numpyro.handlers.condition(model2, data=coefTrue_H1)
#%%with conditioning
...
#%%plot latentaianxiety distrivution as density plot
sns.kdeplot(prior_predictions_H1['LatentAIAnxiety'][0], color='blue', label='LatentAIAnxiety')

#-------------------------------------------------
##Run some prior predicitve checks for H1 is true  
#-------------------------------------------------

#%% create Plot for the prior predictive distribution of WTP_DST when H1 is true as in pregresitration (Figure 9)
latent_anxiety_values = np.mean(prior_predictions_H1['LatentAIAnxiety'], axis=0)
# Define colors to ensure 0 maps to blue
colors = [(0, "green"), (0.5, "blue"), (1, "orange")]
cmap = mcolors.LinearSegmentedColormap.from_list("custom_cmap", colors)

# Calculate min and max of LatentAIAnxiety values, centering 0 around the middle
vmin, vmax = np.min(latent_anxiety_values), np.max(latent_anxiety_values)
norm = mcolors.TwoSlopeNorm(vmin=vmin, vcenter=0, vmax=vmax)  # Center 0 at blue
fig, ax = plt.subplots()

# Plot for each sample of LatentAIAnxiety
for i in range(prior_predictions_H1['LatentAIAnxiety'].shape[1]):
    # Compute dWtpHat
    dWtpHat_DST = 1+(np.mean(prior_predictions_H1['beta_AA']) * latent_anxiety_values[i] 
               + possible_values_TruePerformance_relTo90 * np.mean(prior_predictions_H1['beta_TruePerformance']))
    
    # Get a color for this line based on the normalized LatentAIAnxiety value
    color = cmap(norm(latent_anxiety_values[i]))
    
    # Plot with the corresponding color
    ax.plot(possible_values_TruePerformance_relTo90, dWtpHat_DST, color=color, alpha=1)
...
plt.colorbar(sm, ax=ax, label='LatentAIAnxiety')
#plt.ylim(-3, 3)  # Set the y-axis limits to match the range of dWtpHat
plt.title('Prior Predictive Distribution of WTP_DST_relto90 when H1 is true')
plt.xlabel('Performance (relative to 90)')
plt.ylabel('WTP for DST (relative to 90)')
ax.set_xticks([-5, 0, 5])
ax.set_xticklabels(['-5', '0', '5']) 
ax.set_yticks([-7,-5,-3,-1, 0, 1,3,5,7])
ax.set_yticklabels(['-7','-5','-3','-1', '0', '1','3','5','7'])

# Show the plot
plt.show()


#%%same for the WTP for human
# Define colors to ensure 0 maps to blue
colors = [(0, "green"), (0.5, "blue"), (1, "orange")]
cmap = mcolors.LinearSegmentedColormap.from_list("custom_cmap", colors)
...
# Loop over each sample of LatentAIAnxiety for plotting
for i in range(prior_predictions_H1['LatentAIAnxiety'].shape[1]):
    # Calculate dWtpHat_Human
    # Calculate dWtpHat_Human as an array
    dWtpHat_Human = 1+ np.mean(prior_predictions_H1['beta_TruePerformance']) * possible_values_TruePerformance_relTo90


    # Color for each line based on LatentAIAnxiety
    color = cmap(norm(latent_anxiety_values[i]))

    # Plot with the corresponding color
    ax.plot(possible_values_TruePerformance_relTo90, dWtpHat_Human, color=color, alpha=1)
...
# Add colorbar explicitly to the figure
plt.colorbar(sm, ax=ax, label='LatentAIAnxiety')
plt.ylim(-3, 3)  # Set y-axis limits to match the range of dWtpHat
plt.title('Prior Predictive Distribution of WTP_Human_relto90 when H1 is true')
plt.xlabel('Performance (relative to 90)')
plt.ylabel('WTP for Human (relative to 90)')
ax.set_xticks([-5, 0, 5])
ax.set_xticklabels(['-5', '0', '5'])
ax.set_yticks([-7, -5, -3, -1, 0, 1, 3, 5, 7])
ax.set_yticklabels(['-7', '-5', '-3', '-1', '0', '1', '3', '5', '7'])
...
#%%let's plot for one observation both lines where the individual is algorithm averse, 
# #one hwre the person is neutral and one wher the person is algorithm loving
# Colors for LatentAIAnxiety values 2, 0, -2
# Define the target LatentAIAnxiety values and corresponding labels
target_values = [1, 0, -1]
indices = [np.argmin(np.abs(latent_anxiety_values - target)) for target in target_values]
colors = ["orange", "blue", "green"]  # Choose distinct colors for each target
labels = ["Algorithm Averse", "Algorithm Neutral", "Algorithm Loving"]

fig, ax = plt.subplots()

for idx, color, target_value, label in zip(indices, colors, target_values, labels):
    # Calculate dWtpHat_Human and dWtpHat_DST for each selected observation
    dWtpHat_Human = 1 + np.mean(prior_predictions_H1['beta_TruePerformance']) * possible_values_TruePerformance_relTo90
    dWtpHat_DST = 1 + (np.mean(prior_predictions_H1['beta_AA']) * latent_anxiety_values[idx] 
                       + possible_values_TruePerformance_relTo90 * np.mean(prior_predictions_H1['beta_TruePerformance']))

    # Plot dWtpHat_Human (solid) and dWtpHat_DST (dashed) with the custom label
    ax.plot(possible_values_TruePerformance_relTo90, dWtpHat_DST, color=color, linestyle='solid', alpha=0.5, label=f"WTP_DST when {label}")
    ax.plot(possible_values_TruePerformance_relTo90, dWtpHat_Human, color=color, linestyle='dotted', alpha=0.3, label=f"WTP_Human when {label}")
    
# Customize plot aesthetics

plt.title('Prior Predictive Distribution of WTP_Human and WTP_DST when H1 is true')
plt.xlabel('Performance (relative to 90)')
plt.ylabel('WTP (relative to 90)')
ax.set_xticks([-5, 0, 5])
ax.set_xticklabels(['-5', '0', '5'])
ax.set_yticks([-7, -5, -3, -1, 0, 1, 3, 5, 7])
ax.set_yticklabels(['-7', '-5', '-3', '-1', '0', '1', '3', '5', '7'])

# Add a legend outside below the plot and show plot, decrease font size of legend
plt.legend(loc='upper center', bbox_to_anchor=(0.5, -0.1), shadow=True, ncol=2, fontsize='small')
plt.show()
...
keys_to_take = ['AIA_1', 'AIA_2', 'AIA_3', 'AIA_4', 'AIA_5', 'AIA_6', 'AIA_7', 'AIA_8', 'AIA_9', 
                   'AIA_10', 'AIA_11', 'AIA_12', 'AIA_13', 'AIA_14', 'AIA_15', 'AIA_16', 'SI_1', 'SI_2', 
                   'PE_1', 'PE_2', 'PE_3', 'PE_4', 'EE_1', 'EE_2', 'EE_3', 'BI_1', 'BI_2', 'BI_3', 
                  'WTP_DST_relTo90', 'WTP_H_relTo90']

dict_synthetic_H1 ={**dict_characteristics, **{k: v for k, v in prior_predictions_H1.items() if k in keys_to_take}}
print(dict_synthetic_H1)

#%%save as csv as well
# Flatten the dictionary values if they are multi-dimensional arrays
dict_synthetic_H1_flattened = {key: np.ravel(value) for key, value in dict_synthetic_H1.items()}
# Convert the flattened dictionary to a DataFrame
df_synthetic_H1 = pd.DataFrame(dict_synthetic_H1_flattened)
#%% Save the DataFrame as a CSV file
# Ensure the directory exists
os.makedirs('SyntheticData', exist_ok=True)
df_synthetic_H1.to_csv('SyntheticData/synthetic_data_H1_Version2.csv', index=False)
...
#%% Create the Predictive object for prior predictive sampling when H1 is true
condition_model_NoH1 = numpyro.handlers.condition(model2, data=coefTrue_NoH1)
#%%with conditioning
...
#%%plot density distribution of LatentAIAnxiety
sns.kdeplot(prior_predictions_NoH1['LatentAIAnxiety'][0], color='blue', label='LatentAIAnxiety')

#-------------------------------------------------
##Run some prior predicitve checks for H1 is NOT true
#-------------------------------------------------

#%% create Plot for the prior predictive distribution of WTP_DST when H1 is not true as in pregresitration (Figure 9)
latent_anxiety_values = np.mean(prior_predictions_NoH1['LatentAIAnxiety'], axis=0)
# Define colors to ensure 0 maps to blue
colors = [(0, "green"), (0.5, "blue"), (1, "orange")]
cmap = mcolors.LinearSegmentedColormap.from_list("custom_cmap", colors)
...
for i in range(prior_predictions_NoH1['LatentAIAnxiety'].shape[1]):
    # Compute dWtpHat
    dWtpHat_DST = 1+(np.mean(prior_predictions_NoH1['beta_AA']) * latent_anxiety_values[i] 
               + possible_values_TruePerformance_relTo90 * np.mean(prior_predictions_NoH1['beta_TruePerformance']))
...
plt.colorbar(sm, ax=ax, label='LatentAIAnxiety')
#plt.ylim(-3, 3)  # Set the y-axis limits to match the range of dWtpHat
plt.title('Prior Predictive Distribution of WTP_DST_relto90 when H1 is NOT true')
plt.xlabel('Performance (relative to 90)')
plt.ylabel('WTP for DST (relative to 90)')
ax.set_xticks([-5, 0, 5])
ax.set_xticklabels(['-5', '0', '5']) 
...
for i in range(prior_predictions_NoH1['LatentAIAnxiety'].shape[1]):
    # Calculate dWtpHat_Human
    # Calculate dWtpHat_Human as an array
    dWtpHat_Human = 1+ np.mean(prior_predictions_NoH1['beta_TruePerformance']) * possible_values_TruePerformance_relTo90
...
plt.colorbar(sm, ax=ax, label='LatentAIAnxiety')
plt.ylim(-3, 3)  # Set y-axis limits to match the range of dWtpHat
plt.title('Prior Predictive Distribution of WTP_Human_relto90 when H1 is NOT true')
plt.xlabel('Performance (relative to 90)')
plt.ylabel('WTP for Human (relative to 90)')
ax.set_xticks([-5, 0, 5])
ax.set_xticklabels(['-5', '0', '5'])
...
for idx, color, target_value, label in zip(indices, colors, target_values, labels):
    # Calculate dWtpHat_Human and dWtpHat_DST for each selected observation
    dWtpHat_Human = 1 + np.mean(prior_predictions_NoH1['beta_TruePerformance']) * possible_values_TruePerformance_relTo90
    dWtpHat_DST = 1 + (np.mean(prior_predictions_NoH1['beta_AA']) * latent_anxiety_values[idx] 
                       + possible_values_TruePerformance_relTo90 * np.mean(prior_predictions_NoH1['beta_TruePerformance']))

    # Plot dWtpHat_Human (solid) and dWtpHat_DST (dashed) with the custom label
    ax.plot(possible_values_TruePerformance_relTo90, dWtpHat_Human, color=color, linestyle='dotted', alpha=0.1, label=f"WTP_Human when {label}")
    ax.plot(possible_values_TruePerformance_relTo90, dWtpHat_DST, color=color, linestyle='dashed', alpha=0.1, label=f"WTP_DST when {label}")

# Customize plot aesthetics

plt.title('Prior Predictive Distribution for Algorithm Attitudes')
plt.xlabel('Performance (relative to 90)')
plt.ylabel('WTP (relative to 90)')
ax.set_xticks([-5, 0, 5])
ax.set_xticklabels(['-5', '0', '5'])
...
# Ensure the directory exists
os.makedirs('SyntheticData', exist_ok=True)
df_synthetic_NoH1.to_csv('SyntheticData/synthetic_data_NoH1_Version2.csv', index=False)
...
# %%Instantiate a `MCMC` object using a NUTS sampler
# Set kernel
kernel = NUTS(model2, init_strategy = numpyro.infer.init_to_feasible)
mcmc_synthetic_H1True = MCMC(sampler=kernel,
            num_warmup=1000,
            num_samples=1000,
            num_chains=2)
# Run the MCMC sampler and collect samples
rng_key, rng_key_ = random.split(rng_key)
mcmc_synthetic_H1True.run(rng_key=rng_key,**dict_synthetic_H1)
#%%
mcmc_synthetic_H1True.print_summary()
samples_mcmc_H1 = mcmc_synthetic_H1True.get_samples(group_by_chain=False)
print(samples_mcmc_H1.keys())
...
#%%1. H1 true
rng_key, rng_key_ = random.split(rng_key)
posterior_predictive_H1 = Predictive(model2, samples_mcmc_H1)
posterior_predictions_H1 = posterior_predictive_H1(rng_key, **dict_characteristics)

#%%2. H1 not true
rng_key, rng_key_ = random.split(rng_key)
posterior_predictive_NoH1 = Predictive(model2, samples_mcmc_NoH1)
posterior_predictions_NoH1 = posterior_predictive_NoH1(rng_key, **dict_characteristics)


#%%plot the posterior predictive checks to create plots as in preregsitration (Figure 10)

#%%1. for H1 true
# Define colors to ensure 0 maps to blue
latent_anxiety_values = np.mean(samples_mcmc_H1['LatentAIAnxiety'], axis=0)
# Define colors to ensure 0 maps to blue
colors = [(0, "green"), (0.5, "blue"), (1, "orange")]
cmap = mcolors.LinearSegmentedColormap.from_list("custom_cmap", colors)
...
for i in range(samples_mcmc_H1['LatentAIAnxiety'].shape[1]):
    # Compute dWtpHat
    dWtpHat_DST = 1+(np.mean(samples_mcmc_H1['beta_AA']) * latent_anxiety_values[i] 
               + possible_values_TruePerformance_relTo90 * np.mean(samples_mcmc_H1['beta_TruePerformance']))
...
plt.colorbar(sm, ax=ax, label='LatentAIAnxiety')
#plt.ylim(-3, 3)  # Set the y-axis limits to match the range of dWtpHat
plt.title('Posterior Predictive Distribution of WTP_DST_relto90 when H1 is true')
plt.xlabel('Performance (relative to 90)')
plt.ylabel('WTP for DST (relative to 90)')
ax.set_xticks([-5, 0, 5])
ax.set_xticklabels(['-5', '0', '5']) 
...
for i in range(samples_mcmc_H1['LatentAIAnxiety'].shape[1]):
    # Calculate dWtpHat_Human
    # Calculate dWtpHat_Human as an array
    dWtpHat_Human = 1+ np.mean(samples_mcmc_H1['beta_TruePerformance']) * possible_values_TruePerformance_relTo90
...
plt.colorbar(sm, ax=ax, label='LatentAIAnxiety')
plt.ylim(-3, 3)  # Set y-axis limits to match the range of dWtpHat
plt.title('Posterior Predictive Distribution of WTP_Human_relto90 when H1 is true')
plt.xlabel('Performance (relative to 90)')
plt.ylabel('WTP for Human (relative to 90)')
ax.set_xticks([-5, 0, 5])
ax.set_xticklabels(['-5', '0', '5'])
...
for i in range(samples_mcmc_NoH1['LatentAIAnxiety'].shape[1]):
    # Compute dWtpHat
    dWtpHat_DST = 1+(np.mean(samples_mcmc_NoH1['beta_AA']) * latent_anxiety_values[i] 
               + possible_values_TruePerformance_relTo90 * np.mean(samples_mcmc_NoH1['beta_TruePerformance']))
...
plt.colorbar(sm, ax=ax, label='LatentAIAnxiety')
#plt.ylim(-3, 3)  # Set the y-axis limits to match the range of dWtpHat
plt.title('Posterior Predictive Distribution of WTP_DST_relto90 when H1 is NOT true')
plt.xlabel('Performance (relative to 90)')
plt.ylabel('WTP for DST (relative to 90)')
ax.set_xticks([-5, 0, 5])
ax.set_xticklabels(['-5', '0', '5']) 
...
for i in range(samples_mcmc_NoH1['LatentAIAnxiety'].shape[1]):
    # Calculate dWtpHat_Human
    # Calculate dWtpHat_Human as an array
    dWtpHat_Human = 1+ np.mean(samples_mcmc_NoH1['beta_TruePerformance']) * possible_values_TruePerformance_relTo90
...
plt.colorbar(sm, ax=ax, label='LatentAIAnxiety')
plt.ylim(-3, 3)  # Set y-axis limits to match the range of dWtpHat
plt.title('Posterior Predictive Distribution of WTP_Human_relto90 when H1 is not true')
plt.xlabel('Performance (relative to 90)')
plt.ylabel('WTP for Human (relative to 90)')
ax.set_xticks([-5, 0, 5])
ax.set_xticklabels(['-5', '0', '5'])
...
numPostSamples_H1 = samples_mcmc_H1_grouped[list(samples_mcmc_H1_grouped.keys())[0]].shape[1]
prior_sampling_forPlot = Predictive(model2, num_samples=numPostSamples_H1)
prior_predictions_H1_forPlot = prior_sampling_forPlot(rng_key,**dict_characteristics)
print(prior_predictions_H1_forPlot.keys())
#%% Add one dimension for num_chains
for key in prior_predictions_H1_forPlot.keys():
...
az.plot_trace(mcmc_synthetic_H1True,
              var_names=['beta_AIA_BI','beta_AA',
                          'beta_TruePerformance', 'alpha_idvConst_AIA'],
              figsize=(5,5));
...
numPostSamples_NoH1 = samples_mcmc_NoH1_grouped[list(samples_mcmc_NoH1_grouped.keys())[0]].shape[1]
prior_sampling_forPlot = Predictive(model2, num_samples=numPostSamples_NoH1)
prior_predictions_NoH1_forPlot = prior_sampling_forPlot(rng_key,**dict_characteristics)
print(prior_predictions_NoH1_forPlot.keys())
#%% Add one dimension for num_chains
for key in prior_predictions_NoH1_forPlot.keys():
